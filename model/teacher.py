...
class Teacher(object):
    """
    CNN: X_inputs=[batch_size, 28, 28, 1]
    """
    ...
    def __init__(self, settings):
        ...
        with tf.variable_scope('cnn'):
            ...
            self.flattened = tf.reshape(self.pool2, [-1, 7 * 7 * 64], name='pool2/squeezed')
            self.fc1 = self.fc('fc1', self.flattened, 7 * 7 * 64, 1024,
    ...
    def fc(self, name, x, in_size, out_size, weights_initializer=None, biases_initializer=None, activation=None):
        ...
        with tf.variable_scope(name):
            ...
            h_fc = tf.nn.xw_plus_b(x, w, b,name='output')
            if activation == 'relu':
